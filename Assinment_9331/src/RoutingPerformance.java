		...
		try {
			for (Connection con : connection) {
				...
				if (scheme.equals("SHP") || scheme.equals("SDP")) {
					...
					/*
					 * generate best path which is least loaded each time a
					...
					LLPDijkstraAlgorithm dij = new LLPDijkstraAlgorithm(tempOccup, start);
					Map<Integer, List<Integer>> pathList = dij.getPathListMap();
					route = pathList.get(con.getDestination());
				}
...
class LLPDijkstraAlgorithm {
	 ...
	 * @param start
	 */
	public LLPDijkstraAlgorithm(float[][] graph, int start) {
		this.graph = graph;
		this.start = start;
		this.dimention = graph.length;
		calculate();
	...
	private void calculate() {
		...
		try {
			...
			for (int count = 1; count < dimention; count++) {
				int bridge = -1;
				float cost = Integer.MAX_VALUE/10;
				for (int end = 0; end < dimention; end++) {
					if (visited[end] == 0 && graph[start][end] < cost) {
						bridge = end;
						cost = graph[start][end];
				 ...
				 * than the maximum loads of each route from the start to the
				 * bridge and from the bridge to the end, then replace the old
				 * load with the new load which is less than before.
				 */
				for (int end = 0; end < dimention; end++) {
					if (visited[end] == 0
							&& Math.max(graph[start][bridge], graph[bridge][end]) < graph[start][end]) {
						graph[start][end] = Math.max(graph[start][bridge],
								graph[bridge][end]);
						List<Integer> pathList = pathListMap.get(end);
						List<Integer> bridgePathList = pathListMap.get(bridge);
						pathList.clear();
